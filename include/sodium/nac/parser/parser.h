...
public:
     ...
     *
     * @return std::unique_ptr<Signature> that is the generated function signature AST node.
     */
    std::unique_ptr<Signature> parseSignature();
...
private:
    ...
    Token *nextToken() noexcept; // returns a pointer to the token after token_, token_ is not changed
};
